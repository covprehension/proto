globals [ ;;global parameters
  ...
  max-Ir
  date-contact ;test pour backtracking
]
...
to setup-globals
  ...
  set date-contact 30 ;test backtracking
end
...
to get-in-contact
  ask citizens
  [
  let contacts other citizens in-radius transmission-distance
  set liste-contacts lput contacts liste-contacts
  set liste-contact-dates lput ticks liste-contact-dates
  set infection-contact one-of contacts with [epidemic-state = 1 or epidemic-state = 2]
...
to get-virus
  if ([epidemic-state] of infection-contact = 1  and random-float 1 < probability-transmission)
  or
  ([epidemic-state] of infection-contact = 2 and random-float 1 < probability-transmission-unreported-infected)
  [
        become-infected
        ask infection-contact [set nb-other-infected nb-other-infected + 1]
    ]
end
...
to connect-contacts
  ...
  [
      ...
      [
      ifelse date-i = date-contact
        [ask contacts-i [create-link-with me [set color yellow set thickness 2]]]
        [ask contacts-i [create-link-with me]]]
      set i i + 1
  ]
  ]
...
"R" 1.0 0 -1184463 true "" "if population-size > 0 [plotxy (ticks / nb-step-per-day) (nb-R / population-size  * 100)]"
...
961
304
Date du contact ciblée (en ticks)
date-contact
17
1
11
